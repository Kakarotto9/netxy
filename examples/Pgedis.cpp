...
int main()
{
    ...
    asyncClient.postQuery("INSERT INTO public.kv_data(key, value) VALUES ('netxy5', '{\"hp\":100000}') "
    ...
    asyncClient.hget("heros:netxy", "hp", [](const string& value){
        cout << "hget heros:netxy:" << value << endl;
    });

    asyncClient.hset("heros:netxy", "hp", "{\"hp\":1}", [](bool isOK){
        cout << "hset heros:netxy:" << isOK << endl;
    });

    asyncClient.hmget("heros:netxy", { "hp", "money" }, [](const unordered_map<string, string>& kvs){
        cout << "hmget:" << endl;
        for (auto& kv : kvs)
        {
    ...
    asyncClient.hgetall("heros:netxy", [](const unordered_map<string, string>& kvs){
        cout << "hgetall:" << endl;
        for (auto& kv : kvs)
        {
            cout << kv.first << " : " << kv.second << endl;
